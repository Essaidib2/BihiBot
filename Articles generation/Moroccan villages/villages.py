...
SAVE_MESSAGE = "ⵙⵏⵓⵍⴼⵉⵖ ⴰⵎⴳⵔⴰⴷ"

excel_file_path = 'tarudant.xlsx' #'armed.xlsx'
...
def interlink_page_with_qid(page, lang, qid, namespace):
    ...
    if item.exists():
        ...
        if lang + "wiki" not in sitelinks.keys():
            ...
            item.setSitelink(page, summary=u'Setting sitelink by zgh '+namespace)
...
def create_article(x,template_file):
    ...
    if not str(df.iloc[i, key_dicc('{population}')]).lower()=='x':
        if df.iloc[x, key_dicc('{population}')] > df.iloc[x, key_dicc('{population_2004}')]:
            text_code_source = text_code_source.replace("{pop_change}", "ⵢⵓⵍⵉ")
        elif df.iloc[x, key_dicc('{population}')] < df.iloc[x, key_dicc('{population_2004}')]:
            text_code_source = text_code_source.replace("{pop_change}", "ⵉⴳⴳⵯⵉⵣ")
        else:
            text_code_source = text_code_source.replace("{pop_change}", "ⵓⵔ ⵉⵏⴼⵉⵍ")
        ...
        if df.iloc[x, key_dicc('{families}')] > df.iloc[x, key_dicc('{families_2004}')]:
            text_code_source = text_code_source.replace("{fam_change}", "ⵢⵓⵍⵉ")
        elif df.iloc[x, key_dicc('{families}')] < df.iloc[x, key_dicc('{families_2004}')]:
            text_code_source = text_code_source.replace("{fam_change}", "ⵉⴳⴳⵯⵉⵣ")
        else:
            text_code_source = text_code_source.replace("{fam_change}", "ⵓⵔ ⵉⵏⴼⵉⵍ")
    ...
    # {prov_type}
    text_code_source = text_code_source.replace("{prov_type}", "ⵜⴰⵙⴳⴰ")
